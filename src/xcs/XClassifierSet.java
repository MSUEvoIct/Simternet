 ...
 * Moreover, it provides all necessary different sums and averages of parameters
 * in the set. Finally, it handles addition, deletion and subsumption of
 * classifiers.
 * 
...
public class XClassifierSet implements Serializable {
	 ...
	 * Note that it is static, so that it is not reconstructed each time a new
	 * set is created.
	 */
	 ...
	 */
	public XClassifierSet(AgentData state, XClassifierSet pop, int time,
			int numberOfActions) {
		this.state = state;
		this.parentSet = pop;
		...
		// Check if each action is covered. If not -> generate covering
		// XClassifier and delete if the population is too big
		boolean again;
		do {
			again = false;
			for (int i = 0; i < actionCovered.length; i++)
				if (!actionCovered[i]) {
					XClassifier newCl = new XClassifier(this.numerositySum + 1,
							time, state, i);
			...
			while (pop.numerositySum > XCSConstants.maxPopSize) {
				...
				// update the current match set in case a classifier was deleted
				// out of that
				// and redo the loop if now another action is not covered in the
				// match set anymore.
				int pos = 0;
				if ((cdel != null)
						&& ((pos = this.containsClassifier(cdel)) != -1)) {
					this.numerositySum--;
	...
	/**
	 * Constructs an XClassifierSet out of an arraylist of XClassifiers
	 * (basically just changing data structures)
	 * 
	 * @param matchSet
	 ...
	 */
	public XClassifierSet(XClassifierSet parentSet,
			ArrayList<XClassifier> actionSet) {
		this.numerositySum = 0;
		this.cllSize = 0;
		this.parentSet = parentSet;
		this.clSet = new XClassifier[actionSet.size()];
		int i = 0;
		for (XClassifier x : actionSet)
			this.clSet[i++] = x;
	}
	...
	private XClassifier deleteFromPopulation() {
		double meanFitness = this.getFitnessSum() / this.numerositySum;
		double sum = 0.;
		for (int i = 0; i < this.cllSize; i++)
	...
	/**
	 * Executes action set subsumption. The action set subsumption looks for the
	 * most general subsumer classifier in the action set and subsumes all
	 * classifiers that are more specific than the selected one.
	 * 
	...
	private void doActionSetSubsumption() {
		...
		// If a subsumer was found, subsume all more specific classifiers in the
		// action set
		if (subsumer != null)
			for (int i = 0; i < this.cllSize; i++)
				if (subsumer.isMoreGeneral(this.clSet[i])) {
					int num = this.clSet[i].getNumerosity();
	...
	private double getPredictionSum() {
		...
		for (int i = 0; i < this.cllSize; i++)
			sum += this.clSet[i].getPrediction()
					* this.clSet[i].getNumerosity();
		return sum;
	}
	...
	/**
	 * Increases recursively all numerositySum values in the set and all parent
	 * sets. This function should be called when the numerosity of a classifier
	 * in some set is increased in order to keep the numerosity sums of all sets
	 * and essentially the population up to date.
	 */
	 ...
	 * @see #deleteFromPopulation
	 * @param cl1
	 *            The first classifier generated by the GA.
	 * @param cl2
	 *            The second classifier generated by the GA.
	 * @param cl1P
	 *            The first parent of the two new classifiers.
	 * @param cl2P
	 *            The second classifier of the two new classifiers.
	 */
	private void insertDiscoveredXClassifiers(XClassifier cl1, XClassifier cl2,
	...
	public void printSet() {
		...
		System.out.println("Pre: "
				+ (this.getPredictionSum() / this.numerositySum) + " Fit: "
				+ (this.getFitnessSum() / this.numerositySum) + " Tss: "
				+ (this.getTimeStampSum() / this.numerositySum) + " Num: "
				+ this.numerositySum);
		for (int i = 0; i < this.cllSize; i++)
	...
	public void printSet(PrintWriter pW) {
		pW.println("Averages:");
		pW.println("Pre: " + (this.getPredictionSum() / this.numerositySum)
				+ " Fit: " + (this.getFitnessSum() / this.numerositySum)
				+ " Tss: " + (this.getTimeStampSum() / this.numerositySum)
				+ " Num: " + this.numerositySum);
		for (int i = 0; i < this.cllSize; i++)
	...
	/**
	 * The Genetic Discovery in XCS takes place here. If a GA takes place, two
	 * classifiers are selected by roulette wheel selection, possibly crossed
	 * and mutated and then inserted.
	 * 
	 ...
	 *            The number of actions possible in the environment.
	 */
	public void runGA(int time, AgentData state, int numberOfActions) {
		// Don't do a GA if the theta_GA threshold is not reached, yet
		if ((this.cllSize == 0)
				|| (time - this.getTimeStampAverage() < XCSConstants.theta_GA))
			return;

		this.setTimeStamps(time);

		double fitSum = this.getFitnessSum();
		// Select two XClassifiers with roulette Wheel Selection
		XClassifier cl1P = this.selectXClassifierRW(fitSum);
		XClassifier cl2P = this.selectXClassifierRW(fitSum);
		...
		cl1.setPredictionError(XCSConstants.predictionErrorReduction
				* (cl1.getPredictionError() + cl2.getPredictionError()) / 2.);
		cl1.setFitness(XCSConstants.fitnessReduction
	...
	/**
	 * Tries to subsume a classifier in the current set. This method is normally
	 * called in an action set. If no subsumption is possible the classifier is
	 * simply added to the population considering the possibility that there
	 * exists an identical classifier.
	 * 
	 * @param cl
	 *            The classifier that may be subsumed.
	 * @see #addXClassifierToPopulation
	 */
	private void subsumeXClassifier(XClassifier cl) {
		...
		if (choices.size() > 0) {
			int choice = (int) (XCSConstants.drand() * choices.size());
	 ...
	 * possible it tries to subsume it in the current set.
	 * 
	 * @see #subsumeXClassifier(XClassifier)
	 */
	private void subsumeXClassifier(XClassifier cl, XClassifier cl1P,
			XClassifier cl2P) {
	...
	private void updateFitnessSet() {
		...
		double[] accuracies = new double[this.cllSize];
