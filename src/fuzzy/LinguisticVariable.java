...
public class LinguisticVariable {
	 ...
	 * 
	 * @param name
	 *            Name of this linguistic variable
	 */
	public LinguisticVariable(String name) {
		super();
		this.thisLVname = name;
		this.storage = new java.util.Hashtable();
		this.for_defuzzification = new java.util.Vector();
	}
	 ...
	 * 
	 * @param mFunction
	 *            MembershipFunction to be added.
	 */
	public void add(MembershipFunction mFunction) {
		double[] range = mFunction.getRange();
		this.storage.put(mFunction.getName(), mFunction);

		// Check if there should be a change in min and max support values
		if (range[0] < this.minSupport)
			this.minSupport = range[0];
		if (range[3] > this.maxSupport)
			this.maxSupport = range[3];
	}
	 ...
	 *            Max support value.
	 */
	public void add(String name, double start, double left_top,
			double right_top, double finish) {
		double[] range = { start, left_top, right_top, finish };
		MembershipFunction temp = new MembershipFunction(name, range);
		this.storage.put(name, temp);

		// Check if there should be a change in min and max support values
		if (start < this.minSupport)
			this.minSupport = start;
		if (finish > this.maxSupport)
			this.maxSupport = finish;
	}
	 ...
	 * @exception fuzzyEngine.NoRulesFiredException
	 *                This exception is thrown if no rules have fired for this
	 *                Linguistic Variable.
	 */
	public double defuzzify() throws NoRulesFiredException {
		// Number of rules fired
		int fired = this.for_defuzzification.size();

		if (fired == 0) {
			NoRulesFiredException e = new NoRulesFiredException(
					"No rules was fired for " + this.thisLVname);
			throw e;
		}

		// min and max X (support) values are already calculated in
		// minSupport/maxSupport
		double step = Math.abs((this.maxSupport - this.minSupport) / 100);
		...
		for (int i = 0; i < (fired - 1); i += 3) {
			scaled = this.getMembershipFunctionByName(
					(String) this.for_defuzzification.elementAt(i + 1)).plot(
					this.minSupport, this.maxSupport, 100);

			// Scale and change weight if needed
			double scale = ((Double) this.for_defuzzification.elementAt(i + 2))
					.doubleValue();
			double weight = 1.0;

			String label = (String) this.for_defuzzification.elementAt(i);
			if (label != null) {
				// Look-up label weight in hash
				Object temp = this.labelWeightsHash.get(label);
		...
		for (int i = 0; i < 100; i++) {
			nominator += (this.minSupport + step * i) * sum[i];
			denominator += sum[i];
		}
	 ...
	 * 
	 * @param name
	 *            java.lang.String Name of the membership function.
	 * @return MemebershipFunction
	 */
	public MembershipFunction getMembershipFunctionByName(String name) {
		return (MembershipFunction) this.storage.get(name);
	}
	...
	public double is(String name) {
		...
		if (m != null)
			return m.fuzzify(this.input_value);
		else
			return -1;
	}
	...
	public void reset() {
		// Remove fired rules
		this.for_defuzzification.removeAllElements();

	}

	/**
	 * Store result of a fired rule. This methos is invoked if a rule fired. It
	 ...
	 * 
	 * @param label
	 *            Label of the fired rule, null if rule has no label.
	 * @param name
	 *            Name of the membership function
	 * @param value
	 *            Membership value.
	 */
	public void set(String label, String name, double value)
			throws RulesParsingException {
		// Add label, name and value to the 'for_defuzzyfication' vector
		this.for_defuzzification.addElement(label);

		// Check that this name is a correct name for a MF
		if (this.getMembershipFunctionByName(name) != null)
			this.for_defuzzification.addElement(name);
		else
			throw new RulesParsingException(
					"Wrong name of a membership function");

		this.for_defuzzification.addElement(new Double(value));
	}
